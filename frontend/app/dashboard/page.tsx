  ...
  return (
    <div className="space-y-8">
      ...
      {usageWarning && !bannerDismissed && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        {statCards.map((s) => (
          <div key={s.label} className="glass-card p-3 sm:p-5">
            ...
            <p className={`mt-1 text-2xl sm:text-3xl font-bold ${s.color}`}>{s.value}</p>
          </div>
        ))}
      </div>
          ...
          {gmailConnected ? (
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3">
              ...
              <button
                onClick={scanGmail}
                disabled={scanning}
                className="rounded-lg bg-accent px-4 py-2 text-xs font-semibold text-background transition hover:bg-accent/90 disabled:opacity-50 w-full sm:w-auto min-h-[44px] sm:min-h-0"
              >
            ...
            <a
              href={gmailAuthUrl}
              className="rounded-lg bg-accent px-4 py-2 text-xs font-semibold text-background transition hover:bg-accent/90 text-center min-h-[44px] flex items-center justify-center"
            >
              Connect Gmail
            </a>
          )}
        ...
        {scanResults.length > 0 && (
          <div className="space-y-2 mt-3">
            ...
            {scanResults.map((email, i) => (
              <div key={i} className="flex flex-col sm:flex-row sm:items-start justify-between rounded-lg border border-card-border p-3 gap-2">
